...
<div class="formApp">
	...
	<form method="post" action="pop.php?paso=confirm<?php echo $params;?>">
		<select id='proyecto' name="proyecto"
			onchange='load_tables(this.value);'>
...
<div class="formApp">
	...
	if($_REQUEST['proyecto']=='nuevo'){
		...
		if(file_exists($Carpeta."/panel/base2")){
			...
			while($fichero=$directorio->read()){
				if($fichero!='.' and $fichero!='..'){
					copy($Carpeta."/panel/base2/".$fichero,$this_project['carpeta']."/panel/base2/".$fichero);
				}
			}
			$directorio->close();
		}
		if(file_exists($Carpeta."/panel/base2/apps")){
			...
			while($fichero=$directorio->read()){
				if($fichero!='.' and $fichero!='..'){
					copy($Carpeta."/panel/base2/apps/".$fichero,$this_project['carpeta']."/panel/base2/apps/".$fichero);
				}
			}
			$directorio->close();
		}
	...
	set_params_ini("LOCAL","url_publica", "http://localhost/sistemapanel/".$this_project['carpeta'], $this_project['carpeta']."/panel/config/config.ini" );
	...
	set_params_ini("GENERAL","html_title", '"'.ucfirst($this_project['nombre'])." :: ".utf8_encode("Panel de Administraci�n".'"'), $this_project['carpeta']."/panel/config/config.ini" );
