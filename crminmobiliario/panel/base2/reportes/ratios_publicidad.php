...
$usuarios=select("fecha_creacion,id,nombre","contacto_canales","where 1");
...
if(sizeof($line0)>0){
	...
	unset($row);
	$l=0;//title
	$row[$l][]=array('Atenciones y Ventas por Publicidad',
					 'class=nombre colspan='.(sizeof($intervalos)*3 +16),
					 'class=success');
	...
	//end
	$l++;//footer
	$row[$l][]=array('Total Separación','class="nombre fijo"','class=info');
	// $row[$l][]="";
	foreach($intervalos as $ii=>$la)
	{
	...
	$row[$l][]='';
	$row[$l][]='';	
	$row[$l][]=$total_ventas_separaciones_cancelaciones=$ffv77;
	$row[$l][]=($ffv77)?'100%':'';
	$row[$l][]='';
	$row[$l][]='';
	$row[$l][]='';
	...
	$l++;//footer
	$row[$l][]=array('Total Ventas/Atenciones','class="nombre fijo"','class=info');
	// $row[$l][]="";
	foreach($intervalos as $ii=>$la)
	{
