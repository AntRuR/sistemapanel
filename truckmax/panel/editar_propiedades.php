<?php //á
?><div class="cuadro_codigo bloque" id="blo_objetos" style="clear:left; position:relative; clear:none; float:left; f">
<?php

$otmeObligatorios=array('width_listado','grupo','seccion','alias_grupo','set_fila_fijo','order_by','group','crear_label','por_linea','titulo','width_listado');

$otmeObligatorios2=array();
foreach($otmeObligatorios as $ttoo){ $otmeObligatorios2[$ttoo]='';}
...
if(strlen($rr)>40){ unset($otme[$ww]); $otmeprimeros[$ww]=$rr; }
if(in_array($ww,$otmeObligatorios)){ unset($otmeObligatorios2[$ww]); }
}
...
if($indice!='campos' and !in_array($indice,array_merge($indicesA,array('me','tabla','archivo')))){
...
$campos_1=array();
foreach($objeto_tabla[$_GET['me']] as $indice=>$valor){ if($indice!='campos'){ $campos_1[]=$indice; } }
...
if($SERVER['ARCHIVO']=='maquina.php'){
	...
	foreach(array_values($campos_3) as $propiedad){
		...
		if($sino=='1' or $sino=='0'){ echo "<option value='0'>0</option><option value='1'>1</option>"; } 
		else {
