			...
			foreach($queries as $querie){
				...
				if(in_array($querie['tipo'],array('inp')) ){
					...
					if($querie['select_multiple']=='1'){
						...
						/*if($_GET['debug']==2)*/
						/*prin(array($where01,$where02));*/


						//echo "$primO,$tablaO,$whereO";
						list($idO,$camposO)=explode(",",$primO);

						$oopciones=select(array($idO,"CONCAT_WS(' ',". str_replace(";",",",$camposO) .") as value"),$tablaO,(($whereO1)?$whereO1:"where 1 ").get_extra_filtro_0($tablaO).$where02,($_GET['debug']==2)?0:0);
