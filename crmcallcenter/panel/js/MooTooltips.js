...
var MooTooltips = new Class({
	...
	hide: function(element){
		this.timer = $clear(this.timer);		
		var elProperties = element.retrieve('properties');
		element.morph({'visibility':'hidden','opacity':0,'top': elProperties.leave});
		elProperties.visible = 0;
	},
	
	show: function(){
		this.currentElement.setStyles({'visibility':'hidden','opacity':0,'z-index':100000});
		this.currentElement.morph({'visibility':'visible','opacity':1, 'top':this.fromTop});		
	...
	createContainer: function( sticky ){
		...
		var header = new Element('div', {'class':'dockBottomHeader','html':'<div class="uno"></div><div class="dos"></div><div class="tre"></div>'});
		if( sticky ){
			var closeBtn = new Element('div', { 'class':'sticky_close' }).injectInside(header);
		}
		var message = new Element('div', {'class':'message'});
		var footer = new Element('div', {'class':'dockBottomFooter','html':'<div class="uno"></div><div class="dos"></div><div class="tre"></div>'});
