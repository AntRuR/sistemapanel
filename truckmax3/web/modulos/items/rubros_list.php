...
if($_GET['gru']=='0'){
	...
	$TITULO = "Portada";
	$PORPAG = 20;
	$ENLPAG = "index.php?modulo=items&acc=list&tab=".$PARAMS['this']."&gru=0&pag=";	
...
elseif(!empty($_GET['gru'])) {
	...
	if($_GET['gru']!='1'){
		...
		$PORPAG = 20;
		$ENLPAG = "index.php?modulo=items&acc=list&tab=".$PARAMS['this']."&gru=".$_GET['gru']."&friendly=".$TITULO."&pag=";	
...
if($_GET['gru']!='1'){
	...
	foreach($grupos as $ii=>$grupo){
		foreach($grupo['productos'] as $oo=>$LIS){
			...
			$grupos[$ii]['productos'][$oo]['precios']=render_precios(array('cambio'=>$COMMON['store']['cambio'],'precio'=>$LIS['precio'],'moneda'=>$LIS['moneda']));
