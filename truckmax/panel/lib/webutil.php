...
function web_render_menu($MENU,$obj=NULL,$tag='h3'){
	...
	foreach($MENU as $i=>$men){ 
		...
		foreach($labelA as $ii=>$label){
			if( $men['disabled']=='1' ){
				$html.= "<$tag><a title='".$label."' class='line_".$ii."' href='#' onclick=\"javacript:return false;\">";
				$html.=$label;
				$html.="</a></$tag>";
			} elseif( $men['onclick']!='' ){
				$ShowTab=($obj['id']!='')?"ShowTab('".$obj['id']."','".$iii."');":"";
				$html.= "<$tag><a title='".$label."' class='line_".$ii."' href='#' rel='nofollow' onclick=\"javacript:".$ShowTab.str_replace("\"","'",$men['onclick']).";return false;\">";
				$html.=$label;
				$html.="</a></$tag>";
			} else {
				$html.= "<$tag><a title='".$label."' class='line_".$ii."' ".( ($men['url'] or $men['url']=='')?"href='".$men['url']."'":"" ).">";
				$html.=$label;
				$html.="</a></$tag>";
			}
		}
...
function web_render_form($FORM){
    ...
    if($field['tipo']!='input_hidden' and $field['tipo']!='textarea_hidden'){ ?>
		<?php if($field['before']!=''){ echo "<div class='camps before' ><label class='name'>&nbsp;</label><span>".$field['before']."</strong></span></div>"; } ?>
		<?php if($field['iniciodiv']!=''){ echo "<div id='".$field['iniciodiv']."'>";} ?>	    
		<?php if($field['before_inner']!=''){ echo "<div class='camps before' ><label class='name'>&nbsp;</label><span>".$field['before_inner']."</strong></span></div>"; } ?>
        <?php echo '<div class="camps" id="p_'.$FORM['nombre'].'_'.$field['campo'][0].'">'; ?>
        <?php if($field['label']!=''){ ?>
...
function web_render_edit_toolbar($SELS){
	...
	textarea.PARAMS  { width:580px; height:120px; position:absolute; display:none; z-index:4; padding:3px; box-shadow:0 0 20px #FFF; font-size:10px; font-family:trebuchet MS; background-color:#030112; color:#FFF; white-space: pre; padding-bottom:22px; border:0; margin-bottom:-3px;  }
...
function web_item($item,$ITEM,$debug=0){
	...
	foreach($ITEM as $est2){
		...
		switch($est){
			...
			if($item['url']){ $html.= "<a class=\"$class\" href=\"".$item['url']."\" title=\"".$item['nombre']."\">".$item['nombre']."</a>"; }
			elseif($item['onclick']){ $html.= "<a class=\"$class\" href=\"#\" onclick=\"".$item['onclick'].";return false;\" rel=\"nofollow\" title=\"".$item[$est]."\">".$item['nombre']."</a>"; }			
			} else {
			$html.= "<$tag class=\"$class\" >";
			if($item['url']){ $html.= "<a class=\"$class\" href=\"".$item['url']."\" title=\"".$item['nombre']."\">"; }
			elseif($item['onclick']){ $html.= "<a class=\"$class\" href=\"#\" onclick='".$item['onclick'].";return false;' rel=\"nofollow\" title=\"".$item[$est]."\">"; }				
			$html.= $item['nombre'];
			...
			if($item['url']){ $html.= "<a class=\"$class\" href=\"".$item['url']."\" title=\"".$item[$est]."\">"; }
			$html.= $item[$est];
			if($item['url']){ $html.= "</a>"; }
			} else {			
			$html.=  (!empty($item[$est]))?"<$tag class=\"$class\" >".$item[$est]."</$tag>":"";
			}
			break;
...
function web_verificar_sesion($opc){
	...
	if($login['status']!='1'){		
		if(!is_null($opc['on_false'])){
			...
			redireccionar_a($COMMON['url_login']);
		}
	} else {
...
function web_render_page($this){
	
	$result=select_fila(
		"id,pagina,titulo,texto,foto,foto_descripcion,fecha_creacion"
		,"paginas"
		,"where pagina='".$this."' and  visibilidad='1' order by id asc limit 0,100"
		,0
		,array(
				'carpeta'=>'pag_imas'
				,'tamano'=>'2'
...
function css_render_esquinas($ID,$DIR,$opciones){
	...
	$files=array(
		'long'=>array('barra_arriba','barra_abajo'),
	...
	foreach($files as $type=>$files2){
		...
		foreach($files2 as $file){
			...
			if($file_exists){ 
				...
				switch($type){
	case "long": $derLong=1; $lineas[]=".$ID .$file { background-image: url(\"".$file0."\"); background-repeat:repeat-x; height:".$size[1]."px; }"; break;
...
function get_arbol($niveles,$nivel=0,$id=NULL){
	global $Items;
	//$nivel=($nivel)?$nivel:0;
...
//	prin($params);
	$items= select(
		...
		array(
			'nivel'=>$params[1],
			'url'=>array('url'=>array('index.php?modulo=items&tab=productos&acc=list&fil='.$params[2].'&val={id}&friendly={nombre}')),
	...
	foreach($items as $item){
		...
		if($niveles[$nextnivel]){ 
			get_arbol($niveles,$nextnivel,$item['id']); 
		}
	}	

}

function get_arboles($niveles){
	global $Items;
	$Items=array();
	get_arbol($niveles);
	return $Items;
