...
//$alertas=select("alerta_fecha,estado","ventas_mensajes","where 1");
//prin($alertas)
// SELECT alerta_fecha,DATE(alerta_fecha) FROM ventas_mensajes WHERE  DATE(alerta_fecha) < DATE(NOW()) AND cumplido=0
// SELECT alerta_fecha,DATE(alerta_fecha) FROM ventas_mensajes WHERE  alerta_fecha>NOW() AND DATE(alerta_fecha) = DATE(NOW()) AND cumplido=0
// SELECT alerta_fecha,DATE(alerta_fecha) FROM ventas_mensajes WHERE  alerta_fecha<NOW() AND DATE(alerta_fecha) = DATE(NOW()) AND cumplido=0

// update(array('estado'=>'1'),"ventas_mensajes","where DATE(alerta_fecha) < DATE(NOW()) AND (cumplido=0 or cumplido is null)",0);
// update(array('estado'=>'2'),"ventas_mensajes","where NOW() < alerta_fecha AND DATE(alerta_fecha) = DATE(NOW()) AND (cumplido=0 or cumplido is null)",0);
// update(array('estado'=>'3'),"ventas_mensajes","where alerta_fecha < NOW() AND (cumplido=0 or cumplido is null)",0);
// update(array('estado'=>'4'),"ventas_mensajes","where cumplido=1",0);
...
// $('in_ctabanco').addEvent('change', function(){alert(8);      
// setTimeout('check_dolar();','1000');
// });

// function check_dolar(){
// 	alert(9);
// 	if($('in_moneda').value=='soles'){
// 		alert('soles');
// 	} else {
// 		alert('dolares');		
// 	}
