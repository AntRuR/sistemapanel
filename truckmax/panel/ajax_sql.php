	...
	case "get_quick":
			...
			foreach($items as $lll=>$linea){ 
				...
				foreach($campS as $camP){
					...
					case "com":
						$valorr=$objeto_tabla[$obj]['campos'][$camP]['opciones'][$linea[$camP]];
...
//$item2[$cam]=fecha_formato($ite,($objeto_tabla[$_REQUEST['v_o']]['campos'][$cam]['formato'])?$objeto_tabla[$_REQUEST['v_o']]['campos'][$cam]['formato']:'0b');
	...
	echo (substr($SERVER['browser'],0,2)=='IE')?str_replace("null","\"\"",json_encode($item)):json_encode($item);
...
//				echo $imas[0].",".$imas[1].",".$imas[2].",".$imas[3].",".$id;		
	...
	//			echo $imas[0].",".$imas[1].",".$imas[2].",".$imas[3].",".$id;		
		...
		if($occ!=''){ if(file_exists($occ)){ $ret['oncreate']="1"; $ret['create']=file_get_contents($SERVER['BASE'].$occ."?id=".$ret['id']); } }
	...
	case "update":
		...
		if($oce!=''){ if(file_exists($oce)){ $ret['onedit']="1"; $ret['edit']=file_get_contents($SERVER['BASE'].$oce."?".str_replace(array("where ","'"),array("",""),$_POST['v_d'])); } }
