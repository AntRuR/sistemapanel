...
foreach($datos_tabla['list'] as $lis){
	if(
			(in_array($lis['tipo'],array('inp','hid','fch','fcr','com','html')) and
	...
	{
		$campSlabel[]=$lis['campo'];
		$campS[]=$lis['campo'];
		if($lis['opciones']!=''){
			$liss=explode("|",$lis['opciones']); if($liss['3']!=''){
				$lisss=explode(";",$liss['3']); $lisss2=explode(",",$liss['0']);
				foreach($lisss as $sssil0){
					list($sssil,$sssil2)=explode("+",$sssil0);
					$objeto_tabla[$_GET['me']]['campos'][$sssil]['label']=$sssil2;
					// $sssil=$sssil0;
...
foreach($campSlabel as $ccc=>$cccc){
