...
.foot a { color:#000; marBG_COLOR_3gin-right:5px; text-decoration:none; }
.foot a:hover { text-decoration:underline; }
.bloque_titulo { 
background-color:#<?php echo $vars['GENERAL']['BG_COLOR_3'];?>;
<?php echo css_gradient_vertical("-100%","#".$BG_COLOR_3_OPP,"25%","#".$vars['GENERAL']['BG_COLOR_3']);?>
}
...
background-image: -webkit-gradient(
    ...
    left bottom,
    right top,
    color-stop(1, #<?php echo $vars['GENERAL']['BG_COLOR_3'];?>),
    color-stop(1.5, #<?php echo $BG_COLOR_3_OPP;?>)
);
background-image: -moz-linear-gradient(
    left top,
    #<?php echo $vars['GENERAL']['BG_COLOR_3'];?> 100%,
    #<?php echo $BG_COLOR_3_OPP;?> 150%
);	
...
if($_GET['block']=='form'){
?><style>#inner {height:310px;overflow: auto;}.formulario {border:0;}</style><?php 	
}
if($_GET['block']!='form'){ 
...
location.href='<?php echo (enhay($SERVER['BASE'].$SERVER['URL'],"maquina.php"))?"maquina.php":$SERVER['BASE'].$SERVER['URL'];?>';
}}).send();
}
</script><div><?php
...
if($_COOKIE['admin']=='1'){
	...
	$menus_d['salir_admin'] =  "<a href='maquina.php?accion=borrarcookie'></span>salir</a>";
...
$menus_d['goweb'] = "<a href='$url_publica' target='_top' ></span>ir a web</a>";


if($Open or $Local==1){

	if( $VALIDAR_SESION!='' or $_SESSION['usuario_id']!='' ){
		$menus_d['entrar'] = "<a ". (($script_name=="maquina.php")?"class='selected'":"") ." href='maquina.php?redirhome=1' ></span>entrar</a>";
	} else {	
		$menus_d['master'] = "<a ". (($script_name=="maquina.php")?"class='selected'":"") .
		" style='". ( ($Open)?'color:navy;':'') ."'".
		" href='maquina.php' ></span>master</a>";	
	}
...
if( $VALIDAR_SESION!='' and $_SESSION['usuario_id']!='' ){
	...
	$menus_d['usuario'] = "<a>". select_dato($sesion_login,$tabla_sesion,"where $sesion_id='".$_SESSION['usuario_id']."'") ."</a>";
}
/*
...
if($vars['GENERAL']['DESARROLLO']=='1' and $vars['INTERNO']['ID_PROYECTO']!="0" and !$LOGIN){
	...
	if($Open){ $menus_d[].=  ( ($_SESSION['verdesarrollo']=='1')?'background-color:#09A707;color:#FFF;':'background-color:#DE1010;color:#FFF;'); }
...
echo "<div style='clear:left;position:relative;padding:0 9px;'>";
...
<style>
#titulo_crear { display:none; }
/*#linea_crear { display:none; }*/
#abri_cerrar_crear { display:none; }
.linea_form_mensaje { display:none; }
/*.linea_form label { display:none; }*/
.formulario label { width:80px; }
.formulario { padding:10px 0 10px 3px; }
</style>	
<?php }
