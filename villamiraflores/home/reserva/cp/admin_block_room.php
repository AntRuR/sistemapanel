...
if((isset($_GET['action'])) && ($_GET['action'] == "unblock")){
	...
	$result      = mysql_query("SELECT count(*) as cn from bsi_reservation where  bookings_id=".$booking_id."  and  room_type_id=".$roomtype_id." group by  room_type_id");
	...
	if($num['cn']<=1){
		...
		mysql_query("DELETE brv.* FROM `bsi_reservation` brv, bsi_room br WHERE brv.room_id=br.room_id and brv.`room_type_id`=".$roomtype_id." and br.capacity_id=".$capacity_id." and brv.`bookings_id`=".$booking_id);
