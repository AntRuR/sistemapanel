			...
			foreach($monthNames as $key => $month){ 
				...
				for ($i=0; $i<($maxday+$startday); $i++) {
				   ...
				   if(isset($_POST['submit'])){	
						 ...
						 $result22=mysql_query("SELECT `booking_id` FROM `bsi_bookings` WHERE '".$dt."' between `start_date` and `end_date`");
						 if(mysql_num_rows($result22)){
							 ...
							 while($row=mysql_fetch_assoc($result22)){
								 ...
								 $res66=mysql_query("select rs.room_id from bsi_reservation rs,bsi_room rm 													where rs.room_id=rm.room_ID and rs.room_type_id='".$_POST['roomtype']."' and rs.bookings_id='".$row['booking_id']."' ". $add_query1111."");
							
											
									
										$bookroom+=mysql_num_rows($res66);;
