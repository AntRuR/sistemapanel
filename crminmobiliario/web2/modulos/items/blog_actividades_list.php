...
elseif(!empty($_GET['fil']) and !empty($_GET['val'])) {
	switch($_GET['fil']){
		...
		$TITULO = Titulo_Filtro("Actividades del mes de ",$_GET['val']);
		$PORPAG = 10;	
...
$LISTA = paginacionnumerada(
			...
			,array(
				...
				'foto:atributos'=>array('atributos'=>'bloact_imas,{fecha_creacion},{file},2,150x110,1'),
				'fecha'=>array('fecha'=>array('{fecha}','5')),
				'esquema'=>'foto,fecha,nombre:h2,texto',
			)
        );
...
$LISTA['combo']=array(
			...
			'items'=>select(
				...
				,0
				,array(
				'opcion'=>array('function'=>array('global $Array_Meses; list($yy,$mm)=explode("-","{value}"); echo "Actividades del mes de ".$Array_Meses[$mm]." de ".$yy;')),
