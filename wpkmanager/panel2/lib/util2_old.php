...
//showoncrearsubs
function getStringFilters($out,$ot,$este){
...
function fixEncoding($in_str)
{
  ...
  if($cur_encoding == "UTF-8" && mb_check_encoding($in_str,"UTF-8"))
	...
	*/
	...
	global $link;
	$result = mysqli_query("SHOW COLUMNS FROM ".$tabla);
	// prin($result);
	$count = 0;
	while ($row=mysqli_fetch_row($result)){
		$cnt = 0;
...
function render_foreig_exists($obj0,$linea,$urd){
		...
		foreach($obj2 as $obj){
			...
			$where="where ".$reemplazowhere." ";
   			...
   			if(hay($datos_tabla['tabla'],$where,0)){
				echo "<a style='color:green;font-weight:bold;text-decoration:underline;' rel='{$obj['rel']}' href='{$reemplazo}'>{$obj['text']}</a>";
				// prin ( `<a rel="{$obj['rel']}" href="{$obj['href']}">{$obj['text']}</a>`);
...
function render_foreig_subs($obj0,$linea,$urd,$sub_procesos=null){
					...
					foreach($campS as $camP){
						...
						switch($objeto_tabla[$obj['obj']]['campos'][$camP]['tipo']){
							...
							default:
								...
								case "currency":
								echo '<span class="currency">'.number_format(($linea[$camP]!='')?$linea[$camP]:"", 2, '.', ',').'</span>';
								break;
								default:
								echo ($linea[$camP]!='')?$linea[$camP]:"";
								break;
...
function render_foreig_subs_new($obj0,$id,$urd,$sub_procesos=null){
...
function pre_procesar_tabla($objeto_tabla,$vars){
	...
	*/
	...
	foreach($objeto_tabla_original as $obj=>$ot){
		if( isset($ot['disabled']) && $ot['disabled']=='1'){ $objeto_tabla2[$obj]['disabled']=1; }
		else {
		if(in_array($obj,$Permisos)){
			$objeto_tabla2[$obj]['disabled']=0;
			if(
				isset($objeto_tabla[$obj]['page']) &&
				!($objeto_tabla2[$obj]['page']=='1')
			){ $ooii[]=$obj; }
...
function procesar_lives($lives,$este){
...
function procesar_loads($string,$este){
if($string[strlen($string)-1]=='='){
$string=$string."'+\$v('in_".$este."')+'";
} else {
$eee=explode("[",$string);
foreach($eee as $ee){
if(enhay($ee,"]")){
...
function procesar_loads2($string,$este){
//return $string;
if($string[strlen($string)-1]=='='){
$string=$string."'+json.".$este."+'";
} else {
$eee=explode("[",$string);
foreach($eee as $ee){
if(enhay($ee,"]")){
