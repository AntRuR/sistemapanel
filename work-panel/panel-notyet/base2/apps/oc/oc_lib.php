	...
	foreach($dataRows as $key=>$dataRow){
		...
		if( $dataRows[$key]['tipo_cliente']=='1' ){
			...
			if( $dataRows[$key]['pag_tipo_documento']=='2' ){
				...
				$dataRows[$key]['pag_factura_ruc'] = $dataRows[$key]['pag_factura_ruc'];
		...
		$dataRows[$key]['producto_conversion']   = ($dataRows[$key]['producto_conversion']=='1')?'US$'.$dataRows[$key]['glpgnv_text']:'';

		if($dataRows[$key]['tipo_documento']=='2'){
			$dataRows[$key]['producto_credito_leasing']='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;X';
		}
...
function update_saldos($id_vio){
	...
	// prin($total); exit();
	$documentos=select("importe,id,orden_status,monto","productos_ventas_documentos","where id_grupo=".$id_vio." order by id asc",0);
	$saldo=$total;
	foreach($documentos as $documento){
		if($documento['orden_status']=='7'){
			...
			// prin($saldo);
			// prin($documento['monto']);
			$saldo=$saldo - $documento['monto'];
			// prin($saldo);
...
function get_saldo($id_vio=null){

	if($id_vio==null) return 1;
	$total=dato("producto_precio_facturar","ventas_items_ordenes","where id=".$id_vio);
	// prin($total); exit();
	$documentos=select(
		"importe,id,orden_status,monto",
		"productos_ventas_documentos",
		"where id_grupo=".$id_vio." and orden_status in (3,4)
		order by id asc",0);
	$saldo=$total;
	foreach($documentos as $documento){
		$saldo=$saldo - $documento['monto'];
