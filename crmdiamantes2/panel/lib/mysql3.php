	...
	if($wer['value']!=''){
		...
		if($parametros['porpag']==0){
			...
			if($total>$porpag){
				...
				} else { $prev=($anterior=='')?"":"<a " . enlace($enlace,$onclick,$prev_pag,$var_pag,$procesar_url) . " class='linkarrow'>$anterior</a>";
				}

				if ($total==($finpag+$inicio)) {
					$next="<span class='linkarrowselec'>".$siguiente."</span>";
				} else { $next=($siguiente=='')?"":"<a " . enlace($enlace,$onclick,$next_pag,$var_pag,$procesar_url) . " class='linkarrow' >$siguiente</a>";
				}
...
function prin($array,$bg=NULL){
	...
	if($_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest' and 0){
		...
		echo "<div class=prin><pre".( ($bg)?" style='background-color:".$bg." !important;color:#".oppColour($bg)." !important;'":"").">"; print_r($array); echo "</pre></div>";
	}
