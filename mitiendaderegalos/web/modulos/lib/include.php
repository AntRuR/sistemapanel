...
function render_pedido($carrito,$precio=true,$more){
	...
	$campos_3=3;
	if($more['columnas']){
		foreach($more['columnas'] as $mor){
			$headers[]=$mor['label'];
			$vars[]=$mor['variable'];			
			$campos_extras++;
		}
	}		/*
	print_r($more['columnas']);	print_r($headers);	print_r($vars);	exit();	*/
	$html='';
	$tot=0;
	$html.="<table style='font:inherit;' cellspacing=2 cellpadding=2 width=100%><tbody  bgcolor='#eeeeee' ><tr>";
	foreach($headers as $header){ $html.="<td><b>".$header."</b></td>";	}
	...
	foreach($carrito as $carr){
			...
			$tot=$tot+$sub;
			$html.="<tr>";
			foreach($vars as $var){ $html.="<td>".$carr[$var]."</td>";	}			
			$html.="<td><i>".$carr['n']."</i></td>";
			if($precio){ $html.="<td align=center>".$carr['u']."</td>"; }
			$html.="<td align=center>".$carr['c']."</td>";
			if($precio){ $html.="<td align=right>". $COMMON['datos_root']['simbolo_moneda'] . number_format($sub,2). "</td>"; }
			$html.="</tr>";			
	}
	
	if($more['gastos']){
		$sub=($more['gastos']['m'])?$more['gastos']['m']:0;
		$tot=$tot+$sub;		
		$html.="<tr>";
		$html.="<td><i><b>".$more['gastos']['i']."</b></i></td>";
		if($precio){ $html.="<td align=center>&nbsp;</td>"; }
		for($i=0;$i<$campos_extras;$i++){ $html.="<td align=center>&nbsp;</td>"; }
		$html.="<td align=center>&nbsp;</td>";
		if($precio){ $html.="<td align=right>". $COMMON['datos_root']['simbolo_moneda'] . number_format($sub,2). "</td>"; }
		$html.="</tr>";		
	}
	$campos_3=$campos_extras+$campos_3;
	if($precio){ $html.="<tr><td colspan=".$campos_3." align=right><b>TOTAL</b></td><td align=right><b>". $COMMON['datos_root']['simbolo_moneda'] . number_format($tot ,2). "</b></td></tr>"; }
	$html.="</tbody></table>";
	return $html;
