...
<form name="frm1" action="modificacion-color3.php" method="post" enctype="multipart/form-data">
...
<br/>
  <table border="1">
	...
	if($row_rs_query1['des_col_es']==""){
		...
		?>
			<tr>
			<td><input type="tex" value="<?php echo $row_rs_query1['des_col_es'] ?>" name="<?php echo $row_rs_query1['id_col'] ?>es"/></td><td><input type="tex" value="<?php echo $row_rs_query1['des_col_en'] ?>" name="<?php echo $row_rs_query1['id_col'] ?>en"/></td><td><img src='../imagenes_colores_thumbnails/<?php echo $row_rs_query1['img_col'] ?>'></td>
			<td><a href='ingreso-color4.php?prod=<?php echo $row_rs_query1['id_pro'] ?>&color=<?php echo $row_rs_query1['id_col'] ?>&modi=ok'><img src='imagenes/eliminar.jpg' border='0' /></a></td>
			</tr>
        <?php
		}while($row_rs_query1 = mysql_fetch_array($rs_query1));
		?>        
    <tr>
      <td colspan="4" align="right"><input type="submit" value="MODIFICAR" /></td>
    </tr>
	<?php
    }
    ?>
