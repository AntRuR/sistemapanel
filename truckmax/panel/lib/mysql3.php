		...
		foreach($filas as $fila){
			...
			foreach ($opciones as $dd=>$ff) {
				if( is_array($ff) and sizeof($ff)>0 ){
					...
					if(!is_array($argumentos)){ $argumentos=explode(",",$argumentos); }
					...
					if(function_exists($accion)){ 
						foreach($argumentos as &$aa){ $aa=procesar_llaves($fila2,$aa); }
...
if($parametros['porpag']==0){
	...
	if ($pagin==1) { $prev="<span  class='linkarrowselec'>".$anterior."</span>"; } else { $prev=($anterior=='')?"":"<a " . enlace($enlace,$onclick,$prev_pag,$var_pag,$procesar_url) . " class='linkarrow'>$anterior</a>"; }
	
	if ($total==($finpag+$inicio)) { $next="<span class='linkarrowselec'>".$siguiente."</span>";} else { $next=($siguiente=='')?"":"<a " . enlace($enlace,$onclick,$next_pag,$var_pag,$procesar_url) . " class='linkarrow' >$siguiente</a>"; }
...
function get_imagen( $carpeta, $fecha_bd, $file, $tamano=NULL){
	...
	if((!(strpos($file,"x.")===FALSE)) or ($file=='')) { $img=$USU_IMG_DEFAULT; } else {
	if($tamano==NULL){
...
function prin($array,$bg="#333"){

	 echo "<div style='float:left;width:100%;'><pre style='z-index:1;border-radius:3px;margin-bottom:1px;padding:2px 5px;float:left;text-align:left; position:relative;background-color:".$bg." !important;color:#".oppColour($bg)." !important;font-size:9px;'>"; print_r($array); echo "</pre></div>";
...
function campos_email($CAMPOS,$POST,$ID=NULL,$LABELID=NULL){
	...
	if(!is_null($ID)){ $body_mensaje.="<tr><td nowrap><b>".$LABELID."</b></td><td style='padding-left:10px;'>".$ID."</td></tr>";	}
	
	
	foreach($CAMPOS as $CAMP){
		if($CAMP['enviar']!='0'){
			if(trim($CAMP['campo']['0'])!=''){
				if($CAMP['before']!=''){	$body_mensaje.="<tr><td colspan=2 style='padding-top:6px;padding-bottom:2px;'><b style='color:#999;font-size:14px;text-transform:uppercase;'>".$CAMP['before'].":</b></td></tr>";	}
				switch($CAMP['tipo']){			
					case "input_fecha":	$body_mensaje.="<tr><td nowrap><b>".$CAMP['label'].":</b></td><td style='padding-left:10px;'>".fecha_formato($POST[$CAMP['campo']['0']],2)."</td></tr>"; break;
					case "input_combo": case "input_radio":	$body_mensaje.="<tr><td nowrap><b>".$CAMP['label'].":</b></td><td style='padding-left:10px;'>".
					$CAMP['opciones'][$POST[$CAMP['campo']['0']]].
					"</td></tr>"; break;
					case "input_text": case "input_hidden":	$body_mensaje.="<tr><td nowrap><b>".$CAMP['label'].":</b></td><td style='padding-left:10px;'>".
					(($CAMP['value_email']['0'])?$CAMP['value_email']['0']:$POST[$CAMP['campo']['0']]).
					"</td></tr>"; break;
					case "textarea": case "textarea_hidden": $body_mensaje.="<tr><td colspan=2><b>".$CAMP['label'].":</b></td></tr><tr><td colspan=2>".nl2br($POST[$CAMP['campo']['0']])."</td></tr>"; break;
				}
			}
		}
	}	
...
function dato($campo,$tabla,$donde,$debug=0){
	return select_dato($campo,$tabla,$donde,$debug);	
}
