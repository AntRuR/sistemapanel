...
function resumen($mensajes){
	...
	$html.="Proyecto: ".dato('nombre','productos_items','where id='.$mensaje['id_proyecto'])."<br>";
	...
	$html.="Cliente: <a href='http://crmsche.info/panel/custom/clientes.php?i=".$mensaje['id_cliente']."'>".dato('nombre','clientes','where id='.$mensaje['id_cliente'])." ".dato('apellidos','clientes','where id='.$mensaje['id_cliente'])."</a><br>";
	...
	$html.="Atención: <a href='http://crmsche.info/panel/custom/ventas_items.php?i=".$mensaje['id_item']."'>#".$mensaje['id_item']."</a><br>";
	$html.='<br><br><br>';

		// $html.='#Alerta:'.$mensaje['id_mensaje'].'<br>';
		// $html.='<a href="http://crmsche.info/panel/custom/ventas_items.php?i='.$mensaje['id_item'].'">#Atención:'.$mensaje['id_item'].'</a><br>';
...
//y un cuarto de hora antes del evento de cada alerta.	
	...
	foreach($usuarios as $usuario){
		...
		insert(
			...
			'fecha_programada' =>date('Y-m-d').' 20:00:00',
			'id_cliente'       =>'',
			'id_usuario'       =>$usuario['id'],
			'user'       	    =>$usuario['user'],
			'id_item'          =>'',				
...
foreach($notificaciones_array as $notificaciones){
	...
	if($notificaciones['modulo']=='abonos'){
		...
		$email_cliente=enviar_email(
						...
						,'body'=>$notificaciones['text']
						,'From'=>'no-reply@crmsche.info'
						,'FromName'=>'San Charbel Abonos - CRM Inmobiliario'							
						//,'Logo'=>''
						)
					);
	...
	} elseif($notificaciones['modulo']=='resumen'){
		...
		$email_cliente=enviar_email(
						...
						,'From'=>'no-reply@crmsche.info'
						,'FromName'=>'San Charbel Resúmen - CRM Inmobiliario'							
						//,'Logo'=>''
						)
					);
		...
		$email_cliente=enviar_email(
						...
						,'body'=>$notificaciones['texto']
						,'From'=>'no-reply@crmsche.info'
						,'FromName'=>'San Charbel Seguimiento - CRM Inmobiliario'							
						//,'Logo'=>''
						)
					);
		...
		$email_cliente=enviar_email(
						...
						,'body'=>$notificaciones['texto']
						,'From'=>'no-reply@crmsche.info'
						,'FromName'=>'San Charbel Agenda - CRM Inmobiliario'							
						//,'Logo'=>''
						)
					);
		...
		if($email_cliente){
			...
			update(array('estado'=>'2'),"notificaciones","where id='".$notificaciones['id']."'",0);
