...
case "5":
	//limite: 250 por dia
	$bloque_envios=70; //MUY IMPORTANTE	
	$RELAYS=35;	
	...
	$campain_datos = select_fila(
			...
			,array(          
					...
					,'bloques'=>array('sub_select'=>array(
												...
												,'donde' =>"where id_grupo='{id}' and visibilidad=1 "
												,'debug' =>0
												,'opciones'=>array(
													'carpeta'=>'camfot_imas'
													,'tamano'=>'2'
...
//prin($campains);
	...
	//	echo $campain_datos['enviar_texto'].":<br>";
		if(!empty($item['email'])){		
			$Enviado=sendTo($item['email']['address'],$cuenta['usuario'],$cuenta['clave'],$campain_datos['asunto'],makeHtml($campain_datos,$item),$campain_datos['fronname'],$campain_datos['replayto'],$PROVEEDOR);
...
function sendTo($email,$cuenta,$pass,$asunto,$html,$fromName,$replayTo,$IdProveedor){
	...
	try {
	  ...
	  case "5":
		...
		$mail->SMTPDebug = 1; // 1 tells it to display SMTP errors and messages, 0 turns off all errors and messages, 2 prints messages only.
		$mail->SMTPAuth = true;
		$mail->Host = "mail.calandriatravel.net";
		$mail->Port = 25;	  
...
//abre todo
	//$html.='<table width="850" height="100%" border="0" cellpadding="0" cellspacing="0"><tr><td>';
...
//abre content
//$html.='<table border="0" align="center" cellpadding="0" cellspacing="0"><tr><td  valign="top">';

//abre bloques
			$html.='<table  border="0" cellspacing="0" cellpadding="0" width="800"><tr><td height="23" style="font-family:Arial, Helvetica, sans-serif;font-size:12px" colspan="2">						
			<div align="center">
			...
			if(sizeof($campain['bloques'])>0){
				...
				foreach($campain['bloques'] as $t=>$bloque){
					...
					if($bloque['bloque']=='2'){	
						...
						if($ttt==0){ $html.='<tr>'; }
						$html.='<td>';
						if(trim($bloque['link'])!=''){ $html.='<a style="display:block;font-size:0;" href="'.$bloque['link'].'">'; }
				...
				$html.='<tr><td colspan="2">
					<a href="http://calandriatravel.com">';
					$html.='<img src="http://calandriatravel.com/panel/base2/pieflyer.jpg"  border="0"/>';
					$html.='</a>
				</td></tr>';			
				}
