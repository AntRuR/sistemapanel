	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		echo '<div class="bloque_titulo">';
			...
			*/
			...
			?>class="linkstitu  itr ico_gm" <?php
			?>title="Descargar Base para Group Mail">Exportar Group Mail</a><script>function exportar_gm(){ var url='exportar_gm.php?me=<?php echo $datos_tabla['me'];?>'+(($('ffilter')?'&filter='+$('ffilter').value:'')); location.href=url; }</script> <?php
			}

			if($datos_tabla['exportar_excel']=='1'){

			?><a href="#" onclick="javascript:window.print();return false;" <?php
			?>class="linkstitu  itr ico_Print" <?php
			?>title="Imprimir">Imprimir</a><?php

			?><a href="#" onclick="javascript:exportar_excel();return false;" <?php
			?>class="linkstitu  itr ico_Excel" <?php
			?>title="Descargar Excel">Exportar Excel</a><script>function exportar_excel(){ var url='exportar_excel.php?me=<?php echo $datos_tabla['me'];?>'+(($('ffilter')?'&filter='+$('ffilter').value:'')); location.href=url; }</script> <?php
			}
...
//		prin($datos_tabla);
		...
		*/
		...
		$pagina_items=paginacion(
								array(
									'separador'=>''
									,'porpag'=>($LOCAL and $vars['GENERAL']['mostrar_toolbars'])?5:$datos_tabla['por_pagina']
									,'anterior'=>'&laquo;'
									,'siguiente'=>'&raquo;'
									,'enlace'=>"#"
		...
		$html_filter="<div style='clear:left;'>".$html_filter."</div>";
		...
		echo "<tr><td></td><td class=nombre>mínimo</td><td class=nombre>máximo</td><td class=nombre>media</td></tr>";
...
//							$tbli['label']=str_replace(array('Categoría'),array('Cat'),$tbli['label']);
                            ...
                            echo '<li class="bld '.(($tbli['listclass'])?$tbli['listclass']:'').' '.$nuevoDad.'" >';
							...
							if($tbli['tipo']=='pas'){
                                   ...
                                   ?>><?php
									...
									echo ($_COOKIE['admin']=='1' or 1)?$linea[$tbli['campo']]:$passs;
		...
		?><input type="hidden" id="inner_hidden_num" value='<span class="span2"><span id="span_num" ><?php echo $total_linea; ?></span> <?php echo ($total_linea==1)?$datos_tabla['nombre_singular']:$datos_tabla['nombre_plural']?></span> <span style="font-weight:normal;"><?php echo ($total_linea==$lineassize)?"":"(desde $desde_linea hasta $hasta_linea)"; ?></span>' /><?php
