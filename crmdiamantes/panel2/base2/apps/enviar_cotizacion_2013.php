...
window.moveTo(0,0);
window.resizeTo(1100,750);<?php$NUM_ALERTAS_PENDIENTES=contar('ventas_mensajes','where id_grupo="'.$_GET['id'].'" and alerta_fecha>NOW() ',0);
$id_venta=$_GET['id'];echo "			/*window.addEvent('domready', function(){	new Element('a', {				'id':'imprimir-button',				'html': '". ( ($NUM_ALERTAS_PENDIENTES==0)?"ANTES DEBES PROGRAMAR ALERTA":"ENVIAR") ."',				'title': '". ( ($NUM_ALERTAS_PENDIENTES==0)?"Debe programar una alerta para poder enviar":"click para enviar") ."',				'class':'btn btn-small ". ( ($NUM_ALERTAS_PENDIENTES==0)?"btn-danger":"btn-primary") ."',				'styles': {	 				'position'	: 'absolute',					'right'		: '10px',					'top'		: '10px',					'cursor'	: 'pointer',					'z-index'	: 1				},				'events': {	                'click': function(el){					".( ($NUM_ALERTAS_PENDIENTES==0)?"parent.program_alert($id_venta);parent.initMultiBox.close();":"enviar()" ) ."            		}    		    }	}).inject($(document.body), 'top');});*//*function imprimir(){$0('imprimir-button');window.print();new Request({url:'panel/base2/imprimir_click.php',method:'post',data:{'id_venta':'$id_venta'},onSuccess:function(eee){	parent.ax('recargar');    parent.initMultiBox.close();}});}*/";?>
</script>