...
elseif(!empty($_GET['gru'])) {
	...
	$ID_GRUPO = $_GET['gru'];
	$FILTRO = " and id_tipo='".$ID_GRUPO."' ";
	$ORDEN	= " order by nombre asc";

	$ID_RUBRO = select_dato("id_grupo","productos_tipos","where id='".$ID_GRUPO."'");

	$TITULO = select_dato("nombre","productos_tipos","where id='".$ID_GRUPO."'");
	$SUBTITULO = $TITULO;
...
$LISTA = paginacionnumerada(
        ...
        ,"id,id_grupo,id_tipo,nombre,ficha,fecha_creacion"
        ,"productos_items"
        ,"where 1 and  visibilidad='1' $FILTRO $ORDEN"
       ,0
		,array(
			'tipo'=>array('dato'=>array('campo'=>'nombre','tabla'=>'productos_tipos','where'=>"where id='{id_tipo}'")),
			'marca'=>array('dato'=>array('campo'=>'nombre','tabla'=>'productos_grupos','where'=>"where id='{id_grupo}'")),
