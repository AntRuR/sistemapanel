...
$Adicionales=array('width','style','derecha','listhtml','multicombo');
...
foreach($Quicks as $ttpo=>$Qui){
	if(!isset($valor[$ttpo])){ $valor[$ttpo]=0; };
}
foreach($Adicionales as $ttpo){
	if(!isset($valor[$ttpo])){ $valor[$ttpo]=''; };
}
...
$campos_11=array();
foreach($valor as $indice=>$valo){ $campos_11[]=$indice;  }
...
foreach(array_values($campos_33) as $campo){
	...
	if($sino=='1' or $sino=='0'){ $bbuu.="<option value='0'>0</option><option value='1'>1</option>"; } 
	else{
