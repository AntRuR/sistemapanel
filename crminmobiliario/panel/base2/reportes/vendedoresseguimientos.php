...
$statuses=get_valores('id','nombre','ventas_status');
...
foreach($ventas as $ii=>$venta){
	...
	$ventas[$ii]['status']=($statuses[$venta['id_status']])?$statuses[$venta['id_status']]:$venta['id_status'];

 	$ventas[$ii]['seguimientos']=contar("ventas_mensajes","where id_grupo=".$venta['id']);
...
// prin($ventas);
	...
	foreach($ventas as $ii=>$venta){
		...
		$l++;
		$row[$l][]=["<a target='_blank' href='custom/ventas_items.php?i=".$venta['id']."'>".$venta['id'].'</a>','class=nombre'];
		$row[$l][]=[$venta['usuario'],'class=nombre'];
		$row[$l][]=[$venta['cliente'],'class=nombre'];
		$row[$l][]=[$venta['status'],'class=nombre'];
