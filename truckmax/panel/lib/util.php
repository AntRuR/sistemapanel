...
function procesar_objeto_tabla($array){
	...
	foreach($array['campos'] as $camp){
		...
		if(in_array($camp['tipo'],array('id','fcr','fed','pos','vis','cal','web','page'))){
		...
		if( in_array($camp['listable'],array('1')) or in_array($camp['tipo'],array('id','fcr','vis','pos','cal','web','page'))){
...
foreach($obta['campos'] as $camp){
	...
	if(in_array($camp['tipo'],array('cal'))){
		$Acampos[]="`".$camp['campo']."` int(2) default 0 NOT NULL";
	}	
	if(in_array($camp['tipo'],array('web'))){
		$Acampos[]="`".$camp['campo']."` int(10) default 0 NOT NULL";
	}	
...
$Acampos[]="CREATE TABLE IF NOT EXISTS `".$tablerel."` (`id_".$obta['tabla']."` int(10) NOT NULL DEFAULT '0',`id_".$table9."` int(10) NOT NULL DEFAULT '0',`orden` int(10) NOT NULL DEFAULT '0',PRIMARY KEY (`id_".$obta['tabla']."`,`id_".$table9."`),KEY `id_".$obta['tabla']."` (`id_".$obta['tabla']."`),KEY `id_".$table9."` (`id_".$table9."`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8";
	...
	if(!in_array($camp['campo'],$columnas_existentes)){
		...
		if(in_array($camp['tipo'],array('cal'))){
			$Acampos[]="ALTER TABLE `$nombre_tabla` ADD `".$camp['campo']."`  int(2) default NULL;";
		}		
		if(in_array($camp['tipo'],array('web'))){
			$Acampos[]="ALTER TABLE `$nombre_tabla` ADD `".$camp['campo']."`  int(10) default NULL;";
		}	
			...
			foreach($lineas as $linea){ 
				...
				foreach($campS as $camP){
					...
					case "hid":														
						...
						$adiv0='<span class="tipper" rel="{ajax:\'ajax_sql.php?v_t='.$tablaO.'&v_d='.$idO.'%3D'.$linea[$camP].'&f=get_quick\'}">';
						$adiv1='</span>';
						} else { $adiv0=''; $adiv1=''; }
...
function getinfo($get,$file){
	...
	include($file);
	if(isset($OBJECT[$name])){ 		$ob="OBJECT"; 		if(is_array($OBJECT[$name]['panel'])){ $pans=implode(", ",$OBJECT[$name]['panel']); } }
	if(isset($FORMULARIO[$name])){ 	$ob="FORMULARIO"; 	if(is_array($FORMULARIO[$name]['panel'])){ $pans=implode(", ",$FORMULARIO[$name]['panel']); } }
	if(isset($LISTADO[$name])){ 	$ob="LISTADO"; 		if(is_array($LISTADO[$name]['panel'])){ $pans=implode(", ",$LISTADO[$name]['panel']); } }
			...
			foreach($uno as $kdos=>$dos){
				...
				foreach($dos as $ktres=>$tres){
					...
					foreach($tres as $kcuatro=>$cuatro){				
						if(!is_array($cuatro)){ $EE[$url][$kuno][$kdos][$ktres][$kcuatro]=PPE($url,$cuatro); }
					}}
				}}				
			}}
		}
