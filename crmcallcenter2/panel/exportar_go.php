...
foreach($items as $ii=>$item){
	...
	$filas[$ii]['NOMBRE']          =$item['c_nombre'];
	$filas[$ii]['APELLIDO']        =$item['c_apellido'];
	$filas[$ii]['CORREO']          =$item['c_correo'];
	$filas[$ii]['ARTICULO']        =$item['p_articulo'];
	...
	$filas[$ii]['LINK2']           ='';
	$filas[$ii]['']           		 ='';
	$filas[$ii]['ASESOR']          =$item['a_nombre']." ".$item['a_apellidos'];

		if(trim($item['a_telefono'])         !='') $telefonos[]=$item['a_telefono'];
		if(trim($item['a_telefono_oficina']) !='') $telefonos[]=$item['a_telefono_oficina'];
		if(trim($item['a_celular_claro'])    !='') $telefonos[]=$item['a_celular_claro'];
		if(trim($item['a_celular_movistar']) !='') $telefonos[]=$item['a_celular_movistar'];
		if(trim($item['a_rpm'])              !='') $telefonos[]=$item['a_rpm'];
		if(trim($item['a_rpc'])              !='') $telefonos[]=$item['a_rpc'];

	$filas[$ii]['ASESOR TELEFONO'] = implode(" / ",$telefonos);
