...
elseif(!empty($_GET['fil']) and !empty($_GET['val'])) {
	switch($_GET['fil']){
		...
		$TITULO = Titulo_Filtro("Videos del mes de ",$_GET['val']);
		$PORPAG = 10;	
...
$LISTA = paginacionnumerada(
			...
			,array(
				...
				'foto'=>array('fila'=>array("id,codigo","blog_videos_videos","where id_grupo='{id}' and visibilidad='1' order by id asc limit 0,1",0,
					array('atributos'=>'src="http://i4.ytimg.com/vi/{codigo}/default.jpg" width="140" ')
							)),
...
$LISTA['combo']=array(
			...
			'items'=>select(
				...
				,0
				,array(
					'opcion'=>array('function'=>array('global $Array_Meses; list($yy,$mm)=explode("-","{value}"); echo "Videos del mes de ".$Array_Meses[$mm]." de ".$yy;')),
