...
if($_GET['OT'] == 'productos_ventas'){

	$_GET['L']=dato("id_ventas_item","productos_ventas","where id=".$_GET['L']);
	$_GET['OT']='ventas_items';
	// prin($_GET);
}
...
<body>
	...
	<?php
		...
		// prin($SERVER);
		//CANCELAR DESISTIMIENTO

		//APROBAR DESISTIMIENTO
		// echo '0';
		if(
			1
			){

			if($_SESSION['usuario_id']!='' or $_COOKIE['admin']=='1'){
				// echo '1';
				$sess=fila("id_permisos","usuarios_acceso","where id=".$_SESSION['usuario_id']);
				
				if(in_array($sess['id_permisos'],['1','2','12','10']) or $_COOKIE['admin']=='1'){
					// echo '2';
					if(
						($venta['id_status_pedido']!='' and $venta['id_status_pedido']!='0')
						and
						$venta['id_status']!=$venta['id_status_pedido']
						and
						$venta['id_status']!='10'
						)
					{
						...
						<div class="alert alert-info div_bloque_cuerpo">
							...
							<div>Estado Actual : <strong><?php echo $ventas_status[$venta['id_status']];?></strong></div>
							<div>Estado a Validar : <strong><?php echo $ventas_status[$venta['id_status_pedido']];?></strong></div>

							<div>
							<a class="btn btn-primary " target="_top" href="<?php echo $SERVER['PANEL'];?>/custom/ventas_items.php?i=<?php echo $_GET['L']; ?>&aceptar=1">ACEPTAR</a>
							<a class="btn btn-inverse " target="_top" href="<?php echo $SERVER['PANEL'];?>/custom/ventas_items.php?i=<?php echo $_GET['L']; ?>&cancelar=1">CANCELAR</a>
							</div>
							<div>
								<br>
								<br>
								<a target="_top" href="../../custom/ventas_items.php?i=<?php echo $_GET['L'];?>">ir a atención <?php echo $_GET['L'];?></a>
							</div>
						...
						)
					{
						...
						<div class="alert alert-info div_bloque_cuerpo">
							...
							<div>Desea Anular el desitimiento?</div>

							<div>
							<a class="btn btn-primary " target="_top" href="<?php echo $SERVER['PANEL'];?>/custom/ventas_items.php?i=<?php echo $_GET['L']; ?>&anular=1">ANULAR</a>
							<div>
								<br>
								<br>
								<a target="_top" href="../../custom/ventas_items.php?i=<?php echo $_GET['L'];?>">ir a atención <?php echo $_GET['L'];?></a>
