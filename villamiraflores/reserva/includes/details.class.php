	...
	public function generateBookingDetails() {
		...
		for($i = 0; $i < $selectedRoomsCount; $i++){
			...
			if($this->selectedRooms[$i] > 0){		
				...
				$child_flag2=false;
				$chld_row2=mysql_fetch_assoc(mysql_query("SELECT distinct(`no_of_child`) FROM `bsi_room` WHERE `capacity_id`=".$this->detailVars[$dvarsCtr]['capacityid']." and `roomtype_id`=".$this->detailVars[$dvarsCtr]['roomtypeid'].""));
