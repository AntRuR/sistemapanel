...
include("lib/simple_html_dom.php");
	...
	$productos=select(
					array('id','nombre','ficha','id_tipo'),
					//array('id','nombre','id_grupo'),
					"productos_items",
					"where visibilidad=1 and id_tipo in (select id from productos_tipos where visibilidad=1) order by id_tipo asc, id asc",
					0,
					array(
						'grupo'=>array('fila'=>array('nombre','productos_tipos','where id="{id_tipo}"')),
	...
	$linea=select_fila(
						...
						array(
							...
							'item'		=>array('fila'=>array('nombre,ficha','productos_items','where id="{id_item}"')),							
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			...
			$_POST['msg']=str_replace(array(
									 ...
									 ,array(
											...
											"<a href='http://".(($linea['cuenta']['dominio'])?$linea['cuenta']['dominio']:"www.vehiculos.com.pe")."/index.php?modulo=items&tab=productos_imprimir&acc=file&id_mensaje=".$insertado_mensaje['id']."'>IMPRIMIR</a>",								 
									 		),$_POST['msg']);
	...
	$tbcampos=array(
					...
					'texto'			=>array(
						...
						'variables'		=> array(		
							...
							'FICHA'=>"<span class=\"id_speech\"></span>".str_replace("'","\"",$linea['item']['ficha']),							
