		...
		$daterange = mysql_query("select start_date, end_date, DATE_FORMAT(start_date, '".$bsiCore->userDateFormat."') AS start_date1, DATE_FORMAT(end_date, '".$bsiCore->userDateFormat."') AS end_date1, default_plan from bsi_priceplan where roomtype_id='".$roomtype_id."' group by start_date, end_date");
		  ...
		  if(mysql_num_rows($daterange)){
			while($row_daterange = mysql_fetch_assoc($daterange)){
				$query = mysql_query("select * from bsi_priceplan where start_date='".$row_daterange['start_date']."' and end_date='".$row_daterange['end_date']."' and roomtype_id='".$roomtype_id."' ");
					...
					$i1=$daletetd;			
					  while($row_pp=mysql_fetch_assoc($query)){
						   if($row_pp['capacity_id']==1001){
							  ...
							  $capacity_title=mysql_fetch_assoc(mysql_query("select * from bsi_capacity where id=".$row_pp['capacity_id']));
...
mysql_query("update bsi_configure set conf_value='1' where conf_key='conf_enabled_deposit'");					 
	...
	public function getbsiGallery(){
		...
		$strmsg = "";
		$getArray = array();
		// do not open  ...only if we´repayd the visa complement $appmt = $bsiCore->ClearInput($_POST['type_capacity_id']);
		...
		$result = mysql_query("select * from bsi_gallery where roomtype_id= ".$room_typeid." and capacity_id= ".$capacityid );
		if(mysql_num_rows($result)){
			$html = '<ul>';
			while($row = mysql_fetch_assoc($result)){
